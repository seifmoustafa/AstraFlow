...
4. Create and push a release tag such as `v1.4.2`.
...
- `src/AstraFlow.Contracts/bin/Release/AstraFlow.Contracts.1.4.2.nupkg`
- `src/AstraFlow.Mediator/bin/Release/AstraFlow.Mediator.1.4.2.nupkg`
- `src/AstraFlow.Mapper/bin/Release/AstraFlow.Mapper.1.4.2.nupkg`
- `src/AstraFlow.Mapper.EntityFrameworkCore/bin/Release/AstraFlow.Mapper.EntityFrameworkCore.1.4.2.nupkg`
- `src/AstraFlow.Diagnostics/bin/Release/AstraFlow.Diagnostics.1.4.2.nupkg`
- `src/AstraFlow.Testing/bin/Release/AstraFlow.Testing.1.4.2.nupkg`
- `src/AstraFlow/bin/Release/AstraFlow.1.4.2.nupkg`

For `1.4.2`, inspect the packages before publishing:
...
```xml
<PackageReference Include="AstraFlow.Mediator" Version="1.4.2" />
<PackageReference Include="AstraFlow.Mapper" Version="1.4.2" />
<PackageReference Include="AstraFlow.Mapper.EntityFrameworkCore" Version="1.4.2" />
<PackageReference Include="AstraFlow.Diagnostics" Version="1.4.2" />
```

Use `AstraFlow.Contracts` in shared contract projects that should not reference the mediator runtime:

```xml
<PackageReference Include="AstraFlow.Contracts" Version="1.4.2" />
```

Use `AstraFlow.Testing` only in test projects:

```xml
<PackageReference Include="AstraFlow.Testing" Version="1.4.2" />
```

Use the meta-package only where both mediator and mapper are intentionally needed:

```xml
<PackageReference Include="AstraFlow" Version="1.4.2" />
```
