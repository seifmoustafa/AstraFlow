...
AstraFlow `1.5.1` currently targets:
...
Use AstraFlow `1.5.1` packages from `net8.0`, `net9.0`, `net10.0`, or compatible `netstandard2.0` consumers.
...
For `1.5.1`, inspect the `.nupkg` files and confirm `AstraFlow.Contracts`, the core packages, `AstraFlow.Mapper.Conventions`, and `AstraFlow.Testing` include `lib/netstandard2.0/`, `lib/net8.0/`, `lib/net9.0/`, and `lib/net10.0/`. Confirm `AstraFlow.Mapper.EntityFrameworkCore` includes only `lib/net10.0/`.

Then run:

```powershell
.\scripts\verify-package-install.ps1 -Version 1.5.1
```
