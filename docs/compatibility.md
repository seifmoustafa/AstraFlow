...
AstraFlow `1.8.1` currently targets:
...
| Analyzer package | Analyzer assets ship separately from runtime libraries. | Done in `1.8.1` through `AstraFlow.Analyzers` package assets under `analyzers/dotnet/cs`. |
...
Use AstraFlow `1.8.1` packages from `net8.0`, `net9.0`, `net10.0`, or compatible `netstandard2.0` consumers.
