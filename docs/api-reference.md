...
This reference describes the public AstraFlow `1.12.0` API surface. It is intentionally written from the consumer point of view: what to call, when to call it, what happens, and what fails.
...
`1.12.0` intentionally keeps analyzers source-only and code-fix-free. Source generator work lives in the separate `AstraFlow.Generators` package.
